...
- **작성 시작:** 2026-06-26
- **현재 단계 (2026-06-29):** 수집→요약→주제분류 위키 **가동 중(매일 05:00)**. **정리형(테마)+✍️메모 보존+categories.yaml 코드 완료** → 내일 자동 적용. **코드리뷰 10건 수정 완료.** (pytest 58개)
...
정리형(테마 묶음) + JSON 합성   🔄 코드 완료 → 내일 자동 적용
v2.1 ✍️메모 보존 + categories  🔄 코드 완료 → 내일 자동 적용
코드리뷰 10건 수정 + 정리       ✅ 병합 (pytest 58)
확장(Q&A 웹·웹공개·검증)        💡 백로그
```
> ⏳ **막힌 것은 "정리형 재생성" 하나뿐** — 무료 쿼터 소진. 내일 05:00 cron(쿼터 리셋)에서 정리형+메모+카테고리 자동 적용.
...
| [`10`](10-wiki-themed-plan.md) 정리형 plan | 테마 묶음 + JSON | 🔄 코드완료, 내일 적용 |
| [`11`](11-wiki-v2.1-plan.md) v2.1 plan | 메모보존 + categories.yaml | 🔄 코드완료, 내일 적용 |
